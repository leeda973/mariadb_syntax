...
docker run --name myredis-container -d -p 6379:6379
...
set user:emial:2 hong2@naver.com ex 30
...
# redis는 기본적으로 모든 key:value가 문자열. 
# 그래서 0으로 세팅해도 내부적으로 "0"으로 저장
...
sismember likes:posting:1 abc@naver.com # 내가 좋아요를 눌렀는지 안 눌렀는지 표시
...
# 빈번하게 변경되는 객체값을 저장 시에는 hashes가 성능 효율적
