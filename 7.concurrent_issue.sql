...
-- 2) 터미널을 열어서 selected했을 때, 위 update 변경 사항이 읽히는지 확인(transaction2)
...
commit;
-- 2) 터미널을 열어 아래 코드 실행
insert into author(email) values('ggggggggg@naver.com');


-- repeatable read : 읽기의 일관성 보장 -> lost update 문제 발생 -> 배타lock(배타적 잠금)으로 해결
-- 사용자1이 읽고(1), update 할 때(3), 동시에 사용자2가 읽고(2) 업데이트 하면 lost update 문제가 발생한다.
...
-- select for update를 하게 되면 트랜잭션이 실행되는 동안 lock이 걸리고,
-- 트랜잭션이 종료된 후에 lock이 풀린다. 
DELIMITER //
